...
# Generate QR Code
def code():
    a = ['Abassi', 'Ayed', 'Azzabi', 'Fadhel', 'Fellous', 'Fitoussi', 'Mzali', 'Najar', 'Slim', 'Bouazizi', 'Ben Salem',
         'Ben Romdane', 'Ben yedder',
         'Ached', 'Ben Assile', 'Zbidi', 'Snaidi', 'Fraj', 'Labidi', 'Chaabani', 'Sfar', 'Triki', 'Trabelsi', 'Rtibi',
    ...
    b = ['Marwa', 'Asma', 'Sonia', 'Hajer', 'Khawla', 'Emna', 'Basma', 'Asna', 'Warda', 'Houda', 'Wided', 'Hafsa',
         ...
         'Nesrine', 'Mohamed', 'Afhed', 'Amine', 'Nabil', 'Amidou', 'Moussa', 'Aziz', 'Arsalene', 'Heickel', 'Rayen',
         'Sabri', 'Majdi', 'Slim',
    ...
    c3 = random.choice(c3)
    c = c3, c2, c1
    d = ['Tuins', 'Sfax', 'Sousse', 'Hammamet', 'Tataouine', 'Siliana', 'Kasserine', 'Zarzis', 'Kairouan', 'Tozeur',
         'Madiha', 'Djenouba',
         'Gabes', 'Bizerte', 'Monastir', 'Nabeul', 'Tabarka', 'Le Kef', 'Douz']
    d = random.choice(d)
    e = ['Tuins', 'Sfax', 'Sousse', 'Hammamet', 'Tataouine', 'Siliana', 'Kasserine', 'Zarzis', 'Kairouan', 'Tozeur',
         'Madiha', 'Djenouba',
         'Gabes', 'Bizerte', 'Monastir', 'Nabeul', 'Tabarka', 'Le Kef', 'Douz']
    e = random.choice(e)
    bp = ['Medh v', 'Sfax', 'Tunis centre', 'Soukra', 'Ariana', 'Hmmamet', 'Sousse ville', 'Sousse village', 'Le Kram',
          'Beja', 'Kassrine', 'Siliana', 'Aouina', '']
    bp = random.choice(bp)

    dict = {"nom": a, "prenom": b, "Sexe": genre, "Type": k, "Clef": clef, "Numéro de compte": x, "Nature du compte": n,
            "date de naissance": c, "Lieu de naissance": d, "Adresse": e, "Bureau de poste": bp}
    ...
    def Modif():
        ...
        if (choice == 'nom' or choice == 'NOM' or choice == 'Nom'):
            modifNom()

        if (choice == 'prénom' or choice == 'Prénom' or choice == 'PRENOM'):
            mofifPrenom()

        if (choice == 'genre' or choice == 'Genre' or choice == 'GENRE'):
            modifGenre()
        if (choice == 'type' or choice == 'Type' or choice == 'TYPE'):
            modifType()
        if (choice == 'clef' or choice == 'Clef' or choice == 'CLEF'):
            modifClef()
        if (choice == 'Numéro de compte' or choice == 'numéro de compte' or choice == 'NUMERO DE COMPTE'):
            modifNumerocompte()
        if (choice == 'date de naissance' or choice == 'Date de Naissance' or choice == 'DATE DE NAISSANCE'):
            modifDatenaissance()
        if (choice == 'lieu de naissance' or choice == 'Lieu de Naissance' or choice == 'LIEU DE NAISSANCE'):
            modifLieunaissance()
        if (choice == 'adresse' or choice == 'Adresse' or choice == 'ADRESSE'):
            modifAdress()
        if (choice == 'bureau de poste' or choice == 'BUREAU DE POSTE' or choice == 'Bureau de Poste'):
            modifBureau()
        if (choice == 'aucune' or choice == 'Aucune' or choice == 'AUCUNE'):
            code()
